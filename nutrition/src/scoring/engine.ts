...
export function scoreFood(food: FoodRecord): ScoredFood {
  const axisScores = {
...
export function applyDietaryFlags(
  scored: ScoredFood,
  pattern: DietaryPattern
): ScoredFood {
  const flags: string[] = [...scored.flags];

  if (pattern === "vegan" && scored.food.kingdom === "plant") {
    if (scored.food.per100g.vitaminB12Ug < 0.1) {
      flags.push(
        "No native B12 — plant-only diets require fortification or supplementation"
      );
    }
    if (
      scored.food.per100g.fattyAcids.epa + scored.food.per100g.fattyAcids.dha <
      0.01
    ) {
      flags.push(
        "No EPA/DHA — ALA conversion is limited; consider algae oil or fortification"
      );
    }
    if (scored.scores.eaaCompletenessDigestibility < 70) {
      flags.push(
        "Incomplete plant protein — pair with complementary amino acid sources"
      );
    }
    if (scored.food.bioavailability.ironFactor <= 0.12) {
      flags.push(
        "Low non-heme iron bioavailability — phytate/mineral interactions apply"
      );
    }
  }
