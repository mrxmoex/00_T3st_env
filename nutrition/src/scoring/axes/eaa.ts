...
export function scoreEaa(food: FoodRecord): number {
  const { bioavailability, per100g } = food;
  const completeness =
    bioavailability.completeness ??
...
function computeLimitingRatio(
  aminoAcids: FoodRecord["per100g"]["aminoAcids"],
  proteinG: number
): number {
  if (proteinG <= 0) return 0;
  const proteinMg = proteinG * 1000;
  let limiting = 1;
  for (const key of Object.keys(WHO_REFERENCE_AA) as Array<
    keyof typeof WHO_REFERENCE_AA
  >) {
    const ref = WHO_REFERENCE_AA[key];
    const actual = aminoAcids[key] / proteinMg;
    const ratio = actual / ref;
    if (ratio < limiting) limiting = ratio;
  }
